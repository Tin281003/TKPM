...
public class ProductService {
    ...
    public static List<Product> getData()    {
        ...
        if (statement != null)
            try {
                ResultSet rs = statement.executeQuery("SELECT distinct products.idProduct ,products.productName,typeOfFlow.name, products.size, products.weight, products.description, products.introduction, products.price, STATUS  from products, typeOfFlow where products.idType = typeOfFlow.idType");
                while (rs.next()) {
    ...
    public static Product findById(String id) {
        Product p = null;
        try {
            PreparedStatement ps = con.prepareStatement("SELECT distinct products.idProduct ,products.productName,typeOfFlow.name, products.size, products.weight, products.description, products.introduction, products.price, STATUS  from products, typeOfFlow where products.idType = typeOfFlow.idType and products.idProduct = ?");
            ps.setString(1, id);
